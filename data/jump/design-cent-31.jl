...
function build_model(; I = 1:6, J = 1:2, K = 1:3, pi, x0 = JuMP.Containers.DenseAxisArray(fill(0.5, length(I)), I), y0 = JuMP.Containers.DenseAxisArray(fill(0.0, length(J), length(K)), J, K), l0 = JuMP.Containers.DenseAxisArray(fill(1.0, length(K)), K))
    ...
    @constraint(model, KKT_11, 1 * det + l[1] * (2 * r11 * (y[1, 1] - x[1]) + 2 * r12 * (y[2, 1] - x[2])) == 0)
    @constraint(model, KKT_21, 2 * y[2, 1] * det + l[1] * (2 * r22 * (y[2, 1] - x[2]) + 2 * r12 * (y[1, 1] - x[1])) == 0)
    @constraint(model, KKT_12, - 1 / 4 * det + l[2] * (2 * r11 * (y[1, 2] - x[1]) + 2 * r12 * (y[2, 2] - x[2])) == 0)
    @constraint(model, KKT_22, - 1 * det + l[2] * (2 * r22 * (y[2, 2] - x[2]) + 2 * r12 * (y[1, 2] - x[1])) == 0)
    @constraint(model, KKT_13, 0 + l[3] * (2 * r11 * (y[1, 3] - x[1]) + 2 * r12 * (y[2, 3] - x[2])) == 0)
    @constraint(model, KKT_23, 1 * det + l[3] * (2 * r22 * (y[2, 3] - x[2]) + 2 * r12 * (y[1, 3] - x[1])) == 0)
    @constraint(model, compl[k in K], det >= r11 * (y[1, k] - x[1]) ^ 2 + 2 * r12 * (y[1, k] - x[1]) * (y[2, k] - x[2]) + r22 * (y[2, k] - x[2]) ^ 2 ⟂ l[k])
