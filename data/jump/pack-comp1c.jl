using JuMP
function build_model(; nodes = 1:Nnd, bnd_nodes, int_nodes = nodes diff bnd_nodes, fix_nodes, var_nodes = nodes diff fix_nodes, elements, Omega0, n, h, Nnd, c, i_ref, j_ref, y, u0)
    model = Model()
    ...
    @constraint(model, fix_mem[i in Omega0], u[i] - xi[i] - c * (l[i] - Au[i]) <= 0)
    @constraint(model, slope[i in 1:n], - 3 * h <= a[i-1] - a[i] <= 3 * h)
    @constraint(model, conv[i in 1:n-1], a[i-1] - 2 * a[i] + a[i+1] <= 0)
    @constraint(model, PDE[i in int_nodes], s1[i] == Au[i] - l[i])
    @constraint(model, obst[i in int_nodes], u[i] - xi[i] - c * (l[i] - Au[i]) ⟂ s1[i])
    @objective(model, Min, h / 2 * sum(a[i] + a[i-1] for i in 1:n))
    return model
end
...
function build_model(path::String)
    schema = JuMPConverter.AMPL.DatSchema(
        ...
        [:nodes, :bnd_nodes, :int_nodes, :fix_nodes, :var_nodes, :elements, :Omega0],
    )
    data = if isdir(path)
