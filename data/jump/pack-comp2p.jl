using JuMP
function build_model(; nodes = 1:Nnd, bnd_nodes, int_nodes = nodes diff bnd_nodes, fix_nodes, var_nodes = nodes diff fix_nodes, elements, Omega0, n, h, Nnd, c, r, i_ref, j_ref, y, u0)
    model = Model()
    ...
    @objective(model, Min, h / 2 * sum(a[i] + a[i-1] for i in 1:n) + r * h ^ 2 * sum(u[i] - xi[i] - c * (l[i] - Au[i]) for i in Omega0))
    return model
end
