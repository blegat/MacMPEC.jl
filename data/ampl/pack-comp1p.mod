...
param r := 10^6;                	# penalty parameter
...
# ... obstacle 
var xi {i in nodes} = - 0.04*(x[i]^2 + (y[i]^2 - 0.25)^2);
...
minimize penalty:   h/2*sum{i in 1..n}( a[i] + a[i-1] )
                  + r*h^2*sum{i in Omega0}( u[i] - xi[i] - c*(l[i] - Au[i]) );
