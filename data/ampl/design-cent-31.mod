...
#
# avoid division by small number by multiplying through by det
#
...
subject to
   ...
   KKT_11:  1*det       + l[1] * (   2*r11*(y[1,1] - x[1])
				   + 2*r12*(y[2,1] - x[2]) ) = 0;
   KKT_21:  2*y[2,1]*det+ l[1] * (   2*r22*(y[2,1] - x[2])
				   + 2*r12*(y[1,1] - x[1]) ) = 0;

   KKT_12: -1/4*det     + l[2] * (   2*r11*(y[1,2] - x[1])
				   + 2*r12*(y[2,2] - x[2]) ) = 0;
   KKT_22:  -1*det      + l[2] * (   2*r22*(y[2,2] - x[2])
				   + 2*r12*(y[1,2] - x[1]) ) = 0;

   KKT_13: 0            + l[3] * (   2*r11*(y[1,3] - x[1])
				   + 2*r12*(y[2,3] - x[2]) ) = 0;
   KKT_23: 1*det        + l[3] * (   2*r22*(y[2,3] - x[2])
				   + 2*r12*(y[1,3] - x[1]) ) = 0;
